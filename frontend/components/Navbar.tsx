...
import { User, LogOut, Menu, X, LogIn, UserPlus, LayoutDashboard, Users, Briefcase, Mail } from "lucide-react";

export function Navbar() {
  const { data: session, status } = useSession();
  const [profileData, setProfileData] = useState<{ profileImage: string } | null>(null);
  ...
  async function fetchProfileData() {
    try {
      const res = await fetch(`http://localhost:5000/user/profile/${session?.user?.id}`);
      if (res.ok) {
      ...
      <div className="flex items-center gap-4">
        {session ? (
          <div className="relative hidden md:block" ref={dropdownRef}>
            ...
            >
              <img
                src={profileData?.profileImage ? `http://localhost:5000/uploads/profiles/${profileData.profileImage}` : "/images/placeholder.png"}
                alt="Profile"
          ...
          <div className="hidden md:flex gap-3">
            ...
            <Link href="/signup">
              <Button variant="default" className="flex items-center gap-2 bg-gray-900">
          ...
          {session ? (
            ...
            <div className="flex flex-col gap-3 mt-4 px-6 ">
              <Link href="/login" onClick={() => setMobileMenuOpen(false)}>
                <Button variant="ghost" className="w-full text-left flex items-center gap-2 mt-4 px-6">
                  <LogIn size={18} /> Log In
                </Button>
              </Link>
