...
const InfluencerDashboard = ({ userData }: DashboardProps) => {
	...
	const shouldFetch = Boolean(userProfileId && accessToken);
	...
	useEffect(() => {
		if (userProfileId && accessToken) {
			mutate(); // trigger revalidation
		}
	}, [userProfileId, accessToken, mutate]);

	if (!userProfileId || !accessToken || isValidating) {
		return (
	...
	const rejectedCollabs = collaborations.filter(
		(collab) => collab.status === 'rejected'
	).length;
	...
	return (
		<motion.div
			initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			className="p-8  rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 mb-16"
		>
			{/* Stats Overview */}
			<motion.div
				variants={container}
					...
					<div className="mt-4 space-y-3">
						...
						>
							...
							<span className="font-semibold">
								{(
									totalEarnings / completedCollabs || 0
								).toLocaleString()}{' '}
								PKR
							</span>
						</motion.p>
						...
						>
							...
							<span className="font-semibold">
								{(
									(completedCollabs /
										(completedCollabs + rejectedCollabs)) *
										100 || 0
								).toFixed(1)}
								%
							</span>
							...
							>
								<div>
									<p className="text-lg font-semibold">
										{collab.business.name}
									</p>
									<p className="text-gray-600">
										Budget: {collab.budget} PKR
									</p>
