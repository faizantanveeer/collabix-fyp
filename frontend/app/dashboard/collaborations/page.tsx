...
// Update the path below to the actual location of your UserContext file
...
const Collaborations = () => {
	...
	const pendingRequests = Array.isArray(requestData)
		? requestData.filter((r) => r.status === 'pending')
		: [];

	const activeRequests = Array.isArray(requestData)
		? requestData.filter((r) => r.status === 'accepted')
		: [];

	const completedRequests = Array.isArray(requestData)
		? requestData.filter((r) => r.status === 'completed')
		: [];

	const rejectedRequests = Array.isArray(requestData)
		? requestData.filter((r) => r.status === 'rejected')
		: [];
		...
		>
			...
			<CardHeader>
				<CardTitle className="text-lg">
					...
					From: <span className="font-medium">{request.business.name}</span>
				</p>
				</CardTitle>
