...
const dashboardHandler = async (req, res) => {
	try {
		...
		const user = await User.findOne({ email: req.user.email }).populate(
			'influencerDetails.pastCollaborations'
		);
		...
		const [collaborations, payments, messages, notifications, ratings] =
			await Promise.all([
		...
		const pendingCollaborations = collaborations
			? collaborations.filter((c) => c.status === 'pending')
			: [];
		const activeCollaborations = collaborations
			? collaborations.filter((c) => c.status === 'accepted')
			: [];
		const completedCollaborations = collaborations
		...
		dashboardData = {
			profile: {
				...
				...(role === 'influencer' && {
					influencerDetails: {
						pastCollaborations:
							user.influencerDetails?.pastCollaborations || [],
						ratings: ratings || [],
						totalFollowers:
							user.influencerDetails?.totalFollowers || 0,
					},
				}),
				...(role === 'business' && {
					businessDetails: {
			...
			paymentMethods:
				role === 'influencer'
					? user.influencerDetails?.paymentMethods || []
					: user.businessDetails?.paymentMethods || [],
		};
