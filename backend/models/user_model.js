    ...
    socialLinks: [
      {
        platform: { 
          type: String, 
          enum: ["instagram", "twitter", "youtube", "tiktok", "facebook", "linkedin", "pinterest", "snapchat"],
          required: true
        },
...
userSchema.virtual("totalFollowers").get(function () {
  return this.socialLinks?.reduce((total, link) => total + (link.followers || 0), 0) || 0;
});
...
userSchema.pre('save', function(next) {
  if (this.role === 'influencer') {
    this.influencerDetails.totalFollowers = this.socialLinks?.reduce((total, link) => total + (link.followers || 0), 0) || 0;
  }
  next();
});
